...
): ((data: File) => Promise<BlobObject>) &
  ((data: FileList | HTMLInputElement | File[]) => Promise<BlobObject[]>);
export function useUpload(apiBase: string, options: UploadOptions = {}) {
  ...
  async function upload(
    data: FileList | HTMLInputElement | File[] | File,
  ): Promise<BlobObject[] | BlobObject> {
    ...
    return $fetch(apiBase, {
      ...
      multiple === false || data instanceof File ? (result as any)[0] : result,
    );
  }
