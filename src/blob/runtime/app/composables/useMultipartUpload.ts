...
import type { BlobUploadedPart, BlobObject } from "@syncrolio/nuxthub/blob";
...
export function useMultipartUpload(
  ...
  const { partSize, concurrent, maxRetry, fetchOptions, prefix } = defu(
    options,
    {
  ...
  const create = (file: File) =>
    _fetch<{
      ...
      prefix
        ? joinURL("/create", prefix, file.name)
        : joinURL("/create", file.name),
      {
        method: "POST",
        headers: {
...
export interface UseMultipartUploadOptions {
  ...
  fetchOptions?: Omit<
    FetchOptions,
    "method" | "baseURL" | "body" | "parseResponse" | "responseType"
  >;
}
