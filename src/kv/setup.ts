...
import type { HubConfig, ResolvedKVConfig } from "@syncrolio/nuxthub";
...
export async function setupKV(
  ...
  if (kvConfig.driver === "cloudflare-kv-binding" && kvConfig.namespaceId) {
    addWranglerBinding(nuxt, "kv_namespaces", {
      binding: kvConfig.binding || "KV",
  ...
  if (kvConfig.driver === "upstash" && !deps["@upstash/redis"]) {
    logWhenReady(
      nuxt,
      "Please run `npx nypm i @upstash/redis` to use Upstash Redis KV storage",
      "error",
    );
  }
  if (kvConfig.driver === "redis" && !deps["ioredis"]) {
    logWhenReady(
      nuxt,
      "Please run `npx nypm i ioredis` to use Redis KV storage",
      "error",
    );
  }
  if (hub.hosting.includes("vercel") && kvConfig.driver === "fs-lite") {
    logWhenReady(
      nuxt,
      "Vercel hosting requires a Redis connection. Please set the `REDIS_URL` environment variable. See https://vercel.com/marketplace/category/database",
      "error",
    );
  }
  ...
  nuxt.options.nitro.storage.kv = defu(
    nuxt.options.nitro.storage.kv,
    kvStorageConfig,
  );
...
`;
  ...
  // Write to node_modules/@syncrolio/kv/ for direct imports (workflow compatibility)
  const physicalKvDir = join(
    hub.projectDir,
    "node_modules",
    "@syncrolio",
    "kv",
  ...
  // Write kv.mjs to node_modules/@syncrolio/kv/
  ...
  // Create hub:kv alias to @syncrolio/kv for backwards compatibility
  nuxt.options.alias!["hub:kv"] = "@syncrolio/kv";

  // Add auto-imports for both @syncrolio/kv and hub:kv
  addServerImports({
    name: "kv",
    from: "@syncrolio/kv",
    meta: { description: `The Key-Value storage instance.` },
  });

  // Setup KV Types for hub:kv - point to @syncrolio/kv for type definitions
  addTypeTemplate(
    {
      filename: "hub/kv.d.ts",
      getContents: () => `declare module 'hub:kv' {
  export * from '@syncrolio/kv'
}`,
    },
