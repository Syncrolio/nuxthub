...
export async function getDatabaseMigrationFiles(hub: ResolvedHubConfig) {
  ...
  const migrationsFiles = (await storage.getKeys())
    .map((file) => getMigrationMetadata(file))
    .filter((migration) => migration.dialect === dialect || !migration.dialect);
  ...
  return migrationsFiles.filter((migration) => {
    ...
    if (
      !migration.dialect &&
      migrationsFiles.findIndex(
        (m) => m.name === migration.name && m.dialect === dialect,
      ) !== -1
    ) {
      return false;
    }
...
export async function copyDatabaseMigrationsToHubDir(hub: ResolvedHubConfig) {
  ...
  const srcStorage = createStorage({
    driver: overlayDriver({
      layers: hub.db.migrationsDirs.map((dir) =>
        fsDriver({
          base: dir,
          ignore: [".DS_Store"],
        }),
  ...
  const migrationFiles = (await srcStorage.getKeys()).filter((file) =>
    file.endsWith(".sql"),
  );
  await Promise.all(
...
export async function getDatabaseQueryFiles(hub: ResolvedHubConfig) {
  ...
  const dialect =
    typeof hub.db === "string"
      ? hub.db
      : typeof hub.db === "object" && hub.db !== null && "dialect" in hub.db
        ? hub.db.dialect
        : undefined;

  const queriesFiles = (await storage.getKeys())
    .map((file) => getMigrationMetadata(file))
    .filter((query) => query.dialect === dialect || !query.dialect);
  ...
  return queriesFiles.filter((query) => {
    ...
    if (
      !query.dialect &&
      queriesFiles.findIndex(
        (q) => q.name === query.name && q.dialect === dialect,
      ) !== -1
    ) {
      return false;
    }
...
export async function copyDatabaseQueriesToHubDir(hub: ResolvedHubConfig) {
  ...
  await Promise.all(
    hub.db.queriesPaths.map(async (path) => {
      try {
        ...
        console.error(
          `Failed to read database query file ${path}: ${error.message}`,
        );
      }
    }),
  );
