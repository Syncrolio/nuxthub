...
export async function copyDatabaseAssets(nitro: Nitro, hub: ResolvedHubConfig) {
  ...
  try {
    ...
    await cp(migrationsPath, resolve(outputDir, "db/migrations"), {
      recursive: true,
    });
  ...
  try {
    ...
    await cp(queriesPath, resolve(outputDir, "db/queries"), {
      recursive: true,
    });
 ...
 */
export async function applyBuildTimeMigrations(
  nitro: Nitro,
...
export async function buildDatabaseSchema(
  buildDir: string,
  {
    relativeDir,
    alias,
  }: { relativeDir?: string; alias?: Record<string, string> } = {},
) {
