...
import type { ResolvedHubConfig } from "@syncrolio/nuxthub";
import {
  AppliedDatabaseMigrationsQuery,
  getCreateMigrationsTableQuery,
  splitSqlQueries,
} from "./utils";
import {
  useDatabaseMigrationsStorage,
  getDatabaseMigrationFiles,
  useDatabaseQueriesStorage,
  getDatabaseQueryFiles,
} from "./storage";
...
export async function applyDatabaseMigrations(hub: ResolvedHubConfig, db: any) {
  ...
  const getRows = (result: any) =>
    (dialect === "mysql"
      ? result[0]
      : result.results || result.rows || result) || [];

  const createMigrationsTableQuery = getCreateMigrationsTableQuery({
    dialect: hub.db.dialect,
  });
  ...
  if (!import.meta.dev) {
    log.info(
      `Found ${appliedRows.length} applied migration${appliedRows.length === 1 ? "" : "s"}`,
    );
  }
  ...
  for (const migration of pendingMigrations) {
    ...
    try {
      log.debug(
        `Applying database migration \`${getRelativePath(join(hub.dir!, "db/migrations", migration.filename))}\`...`,
      );
      ...
      const message = error.cause?.message || error.message;
      log.error(
        `Failed to apply migration \`${getRelativePath(join(hub.dir!, "db/migrations", migration.filename))}\`\n${message}`,
      );
      if (message?.includes("already exists")) {
        log.info(
          `To mark this migration as applied, run \`npx nuxt db mark-as-migrated ${migration.name}\``,
        );
    ...
    log.success(
      `Database migration \`${getRelativePath(join(hub.dir!, "db/migrations", migration.filename))}\` applied`,
    );
  }
...
export async function applyDatabaseQueries(hub: ResolvedHubConfig, db: any) {
  ...
  for (const queryFile of queriesFiles) {
    ...
    try {
      log.debug(
        `Applying database query \`${getRelativePath(join(hub.dir!, "db/queries", queryFile.filename))}\`...`,
      );
      for (const query of queries) {
        await db[execute](sql.raw(query));
      }
      ...
      const message = error.cause?.message || error.message;
      log.error(
        `Failed to apply query \`${getRelativePath(join(hub.dir!, "db/queries", queryFile.filename))}\`\n${message}`,
      );
      return false;
    }
    ...
    !import.meta.dev &&
      log.success(
        `Database query \`${getRelativePath(join(hub.dir!, "db/queries", queryFile.filename))}\` applied`,
      );
  }
