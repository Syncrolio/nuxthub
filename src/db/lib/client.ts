...
import type { ResolvedDatabaseConfig } from "@syncrolio/nuxthub";
import type { Casing } from "drizzle-orm";

async function createD1HttpClient(
  accountId: string,
  databaseId: string,
  apiToken: string,
  casing?: string,
...
export async function createDrizzleClient(
  ...
  if (driver === "d1" || driver === "d1-http") {
    ...
    const accountId =
      connection?.accountId || process.env.NUXT_HUB_CLOUDFLARE_ACCOUNT_ID;
    const databaseId =
      connection?.databaseId || process.env.NUXT_HUB_CLOUDFLARE_DATABASE_ID;
    const apiToken =
      connection?.apiToken || process.env.NUXT_HUB_CLOUDFLARE_API_TOKEN;
    if (!accountId || !databaseId || !apiToken) {
      throw new Error(
        "D1 CLI commands require Cloudflare API credentials. Set NUXT_HUB_CLOUDFLARE_ACCOUNT_ID, NUXT_HUB_CLOUDFLARE_DATABASE_ID, and NUXT_HUB_CLOUDFLARE_API_TOKEN, or use `npx wrangler d1 migrations apply <DATABASE_NAME>` instead.",
      );
    }
    ...
    return drizzle({
      connection: { ...connection, onnotice: () => {} },
      casing,
    });
