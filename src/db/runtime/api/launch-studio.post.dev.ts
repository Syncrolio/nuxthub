...
import { schema } from "@syncrolio/db";
...
export default eventHandler(async (event) => {
  ...
  if (driver === "d1") {
    ...
    if (!binding) {
      throw new Error(
        "D1 binding not found. Make sure you are running with `wrangler dev` or the D1 binding is available.",
      );
    }
    ...
    // @ts-expect-error - dynamically generated
    const client = await import("@syncrolio/db").then((m) => m.client);
