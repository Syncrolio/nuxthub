...
import {
  copyDatabaseMigrationsToHubDir,
  copyDatabaseQueriesToHubDir,
  copyDatabaseAssets,
  applyBuildTimeMigrations,
  getDatabaseSchemaPathMetadata,
  buildDatabaseSchema,
} from "./lib";
...
export async function resolveDatabaseConfig(
  nuxt: Nuxt,
  hub: HubConfig,
): Promise<ResolvedDatabaseConfig | false> {
  if (!hub.db) return false;

  let config = typeof hub.db === "string" ? { dialect: hub.db } : hub.db;
  config = defu(config, {
    migrationsDirs: getLayerDirectories(nuxt).map((layer) =>
      join(layer.server, "db/migrations"),
    ),
    queriesPaths: [],
  ...
  switch (config.dialect) {
    case "sqlite": {
      ...
      if (config.driver === "d1-http") {
        ...
        if (
          !config.connection?.accountId ||
          !config.connection?.apiToken ||
          !config.connection?.databaseId
        ) {
          throw new Error(
            "D1 HTTP driver requires NUXT_HUB_CLOUDFLARE_ACCOUNT_ID, NUXT_HUB_CLOUDFLARE_API_TOKEN, and NUXT_HUB_CLOUDFLARE_DATABASE_ID environment variables",
          );
        }
        break;
      }
      ...
      if (
        hub.hosting.includes("cloudflare") &&
        !nuxt.options.dev &&
      ...
      config.driver ||= "libsql";
      config.connection = defu(config.connection, {
        url: `file:${join(hub.dir!, "db/sqlite.db")}`,
      });
    ...
    case "postgresql": {
      ...
      if (
        hub.hosting.includes("cloudflare") &&
        config.connection?.hyperdriveId &&
        !config.driver
      ) {
      ...
      if (
        config.applyMigrationsDuringBuild &&
        config.driver &&
        ["neon-http", "postgres-js"].includes(config.driver) &&
        !config.connection.url
      ) {
        throw new Error(
          `\`${config.driver}\` driver requires \`DATABASE_URL\`, \`POSTGRES_URL\`, or \`POSTGRESQL_URL\` environment variable when \`applyMigrationsDuringBuild\` is enabled`,
        );
      }
...
export async function setupDatabase(
  ...
  const { dialect, driver, connection, migrationsDirs, queriesPaths } =
    hub.db as ResolvedDatabaseConfig;

  logWhenReady(
    nuxt,
    `\`hub:db\` using \`${dialect}\` database with \`${driver}\` driver`,
    "info",
  );

  if (driver === "d1" && connection?.databaseId) {
    addWranglerBinding(nuxt, "d1_databases", {
      binding: "DB",
  ...
  if (["postgres-js", "mysql2"].includes(driver) && connection?.hyperdriveId) {
    ...
    addWranglerBinding(nuxt, "hyperdrive", {
      binding,
  ...
  if (!deps["drizzle-orm"] || !deps["drizzle-kit"]) {
    logWhenReady(
      nuxt,
      "Please run `npx nypm i drizzle-orm drizzle-kit` to properly setup Drizzle ORM with NuxtHub.",
      "error",
    );
  }
  if (driver === "postgres-js" && !deps["postgres"]) {
    logWhenReady(
      nuxt,
      "Please run `npx nypm i postgres` to use PostgreSQL as database.",
      "error",
    );
  } else if (driver === "neon-http" && !deps["@neondatabase/serverless"]) {
    logWhenReady(
      nuxt,
      "Please run `npx nypm i @neondatabase/serverless` to use Neon serverless database.",
      "error",
    );
  } else if (driver === "pglite" && !deps["@electric-sql/pglite"]) {
    logWhenReady(
      nuxt,
      "Please run `npx nypm i @electric-sql/pglite` to use PGlite as database.",
      "error",
    );
  } else if (driver === "mysql2" && !deps.mysql2) {
    logWhenReady(
      nuxt,
      "Please run `npx nypm i mysql2` to use MySQL as database.",
      "error",
    );
  } else if (driver === "libsql" && !deps["@libsql/client"]) {
    logWhenReady(
      nuxt,
      "Please run `npx nypm i @libsql/client` to use SQLite as database.",
      "error",
    );
  }
...
async function generateDatabaseSchema(nuxt: Nuxt, hub: ResolvedHubConfig) {
  ...
  const getSchemaPaths = async () => {
    const schemaPatterns = getLayerDirectories(nuxt)
      .map((layer) => [
      ...
      .flat();
    let schemaPaths = await glob(schemaPatterns, {
      absolute: true,
      onlyFiles: true,
    });

    await nuxt.callHook("hub:db:schema:extend", {
      dialect,
      paths: schemaPaths,
    });
  ...
  if (nuxt.options.dev && !nuxt.options._prepare) {
    ...
    const watchDirs = getLayerDirectories(nuxt).map((layer) =>
      resolveFs(layer.server, "db"),
    );
    ...
    watcher.on("all", async (event, path) => {
      if (
        ...
        !path.endsWith(`db/schema.${dialect}.ts`) &&
      ...
      log.info(
        `Database schema ${event === "add" ? "added" : event === "unlink" ? "removed" : "changed"}: \`${relative(nuxt.options.rootDir, path)}\``,
      );
      log.info(
        "Make sure to run `npx nuxt db generate` to generate the database migrations.",
      );
      ...
      // Also copy to node_modules/@syncrolio/db/ for workflow compatibility
  ...
  addTemplate({
    ...
    getContents: () =>
      `${schemaPaths.map((path) => `export * from '${path}'`).join("\n")}`,
    write: true,
  });
  ...
  nuxt.hooks.hookOnce("app:templatesGenerated", async () => {
    ...
    // Then copy to node_modules/@syncrolio/db/ for workflow compatibility
    const physicalDbDir = join(
      hub.projectDir,
      "node_modules",
      "@syncrolio",
    ...
    try {
      ...
      const nuxtDirSource = join(
        nuxt.options.rootDir,
        ".nuxt/hub/db/schema.d.mts",
      );
  ...
  nuxt.options.alias ||= {};
  // Create hub:db:schema alias to @syncrolio/db/schema for backwards compatibility
  addTypeTemplate(
    {
  ...
  nuxt.options.alias["hub:db:schema"] = "@syncrolio/db/schema";
}

async function setupDatabaseClient(nuxt: Nuxt, hub: ResolvedHubConfig) {
  const { dialect, driver, connection, mode, casing, replicas } =
    hub.db as ResolvedDatabaseConfig;

  const postgresOpts = (() => {
    if (driver !== "postgres-js" || !connection)
      return "{ onnotice: () => {} }";
    const { url: _, ...rest } = connection as Record<string, unknown>;
    return Object.keys(rest).length
      ? `{ onnotice: () => {}, ...${JSON.stringify(rest)} }`
      : "{ onnotice: () => {} }";
  })();
  ...
  // Setup Database Types for hub:db - point to @syncrolio/db for type definitions
  const databaseTypes = `declare module 'hub:db' {
  export * from '@syncrolio/db'
}`;
  ...
  const modeOption =
    dialect === "mysql" ? `, mode: '${mode || "default"}'` : "";
...
`;
  ...
  if (driver === "neon-http") {
    const urlExpr = connection.url
      ? `'${connection.url}'`
      : `process.env.POSTGRES_URL || process.env.POSTGRESQL_URL || process.env.DATABASE_URL`;
...
`;
  ...
  if (
    ["postgres-js", "mysql2"].includes(driver) &&
    hub.hosting.includes("cloudflare") &&
...
${
  ...
  await writeFile(
    ...
    drizzleOrmContent.replace(
      /from '\.\/db\/schema\.mjs'/g,
      "from './schema.mjs'",
    ),
  );
...
`;
  ...
  try {
    ...
    const schemaExists = await stat(schemaPath)
      .then((s) => s.size > 20)
      .catch(() => false);
    const schemaDtsExists = await stat(schemaDtsPath)
      .then((s) => s.size > 20)
      .catch(() => false);
    ...
    throw new Error(
      `Failed to create @syncrolio/db package files: ${(error as Error).message}`,
    );
  }

  // Create hub:db alias to @syncrolio/db for backwards compatibility
  nuxt.options.alias!["hub:db"] = "@syncrolio/db";

  // Add auto-imports for both @syncrolio/db and hub:db
  addServerImports({
    name: "db",
    from: "@syncrolio/db",
    meta: { description: `The ${driver} database client.` },
  });
  addServerImports({
    name: "schema",
    from: "@syncrolio/db",
    meta: { description: `The database schema object` },
  });
}
