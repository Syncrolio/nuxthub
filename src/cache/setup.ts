...
export async function setupCache(
  ...
  if (
    cacheConfig.driver === "cloudflare-kv-binding" &&
    cacheConfig.namespaceId
  ) {
    addWranglerBinding(nuxt, "kv_namespaces", {
      binding: cacheConfig.binding || "CACHE",
      id: cacheConfig.namespaceId,
    });
  }
  ...
  nuxt.options.nitro.storage.cache = defu(
  ...
  if (nuxt.options.dev) {
    ...
    nuxt.options.nitro.devStorage.cache = defu(
  ...
  logWhenReady(
    nuxt,
    `\`hub:cache\` using \`${cacheConfig.driver.split("/").pop()}\` driver`,
  );
}
