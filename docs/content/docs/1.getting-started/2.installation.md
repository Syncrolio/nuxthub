...
This command will install `@syncrolio/nuxthub` as dependency and add it to your `modules` section of your `nuxt.config`.
...
::field-group
::field{name="blob" type="boolean | BlobConfig"}
...
::field{name="cache" type="boolean | CacheConfig"}
...
::field{name="db" type="'postgresql' | 'sqlite' | 'mysql' | DatabaseConfig"}
...
::field{name="kv" type="boolean | KVConfig"}
...
You can also use the latest features and bug fixes (commited on the `main` branch) by installing the [nightly tag](https://www.npmjs.com/package/@syncrolio/nuxthub?activeTab=versions):

```diff [package.json]
{
  "Dependencies": {
-   "@syncrolio/nuxthub": "^1.0.0"
+   "@syncrolio/nuxthub": "npm:@syncrolio/nuxthub@nightly"
  }
}
```
