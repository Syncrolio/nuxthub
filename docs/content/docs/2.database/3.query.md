...
### Recommended: `@syncrolio/db`

Use `@syncrolio/db` to import the database client. This works everywhere, including in Nuxt server routes and external bundlers like [Workflow](https://useworkflow.dev):
...
The `@syncrolio/db` import works seamlessly with external tools that bundle your code independently, such as [Workflow](https://useworkflow.dev).
...
1. NuxtHub generates the database client at `node_modules/@syncrolio/db/`
...
The `@syncrolio/db` package is auto-generated during development and build. You don't need to add it to your `package.json`.
::
