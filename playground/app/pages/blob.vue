...
import type { BlobObject } from "@syncrolio/nuxthub/blob";
...
async function addFile() {
  ...
  try {
    ...
    if (uploadedFiles.length > 0) {
      toast.add({
        title: `File${uploadedFiles.length > 1 ? "s" : ""} uploaded.`,
      });
    }
    newFilesValue.value = [];
  } catch (err: any) {
    const title =
      err.data?.data?.issues?.map((issue: any) => issue.message).join("\n") ||
      err.message;
...
async function deleteFile(pathname: string) {
  try {
    ...
    blobData.value!.blobs = blobData.value!.blobs!.filter(
      (t: BlobObject) => t.pathname !== pathname,
    );
  ...
  <UCard @submit.prevent="addFile">
    <div class="flex">
      <UFieldGroup class="flex-1">
        <UInput
          :model-value="
            newFilesValue?.map((file: File) => file.name).join(', ')
          "
          name="fileValue"
          disabled
          class="flex-1"
    ...
    <div
      v-if="folders?.length || prefixes?.length"
      class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2 mt-4"
    >
      <UButton
    ...
    >
      ...
      >
        ...
        <div
          v-else
          class="h-36 w-full flex items-center justify-center p-2 text-center"
        >
        ...
        >
          ...
          <div class="flex items-center justify-between gap-1">
            ...
            <span class="text-xs">{{
              file.size
                ? `${Math.round((file.size / Math.pow(1024, 2)) * 100) / 100}MB`
                : "-"
            }}</span>
          </div>
