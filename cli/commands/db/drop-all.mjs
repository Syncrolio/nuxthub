...
import { createDrizzleClient } from "@syncrolio/nuxthub/db";
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    consola.warn(
      "This command will drop all tables. ALL DATA STORED IN THE DATABASE WILL BE LOST!",
    );

    if (args.force) {
      consola.warn(
        "Skipping confirmation prompt due to --force flag. Dropping all tables without confirmation.",
      );
    } else {
      const confirmation = await consola.prompt(
        'Type "confirm" to drop all tables:',
        {
          type: "text",
          placeholder: "confirm",
          cancel: "null",
    ...
    if (dialect === "postgresql") {
      ...
      if (schemas.length === 0) {
        ...
        consola.debug(
          `Found ${schemas.length} schema${schemas.length === 1 ? "" : "s"} to clear`,
        );
        ...
        for (const schema of schemas) {
          try {
            ...
            consola.error(
              `Failed to clear schema \`${schema}\`: ${error.message}`,
            );
          }
        }
      }
    ...
    try {
      ...
      tables = getRows(result).map(
        (row) =>
          row.name || row.table_name || row.tablename || Object.values(row)[0],
      );
    } catch (error) {
    ...
    if (tables.length === 0) {
      ...
      consola.info(
        `Found ${tables.length} table${tables.length === 1 ? "" : "s"} to drop`,
      );
