...
import { buildDatabaseSchema } from "@syncrolio/nuxthub/db";
...
export default defineCommand({
  ...
  args: {
    custom: {
      type: "boolean",
      description:
        "Whether to generate an empty migration file for custom SQL.",
      required: false,
    },
    name: {
      type: "string",
  ...
  async run({ args }) {
    ...
    await buildDatabaseSchema(join(options.cwd, ".nuxt"), {
      relativeDir: cwd,
      alias,
    });
    ...
    consola.info(
      "Run `npx nuxt dev` or run `npx nuxt db migrate` to apply the migrations.",
    );
  },
});
