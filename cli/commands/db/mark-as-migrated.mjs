...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    consola.info(
      `Database: \`${dialect}\` with \`${hubConfig.db.driver}\` driver`,
    );
    ...
    consola.info(
      `Found \`${localMigrations.length}\` local migration${localMigrations.length === 1 ? "" : "s"}`,
    );
    consola.debug(
    ...
    if (
      args.name &&
      !localMigrations.find((migration) => migration.name === args.name)
    ) {
    ...
    const getRows = (result) =>
      dialect === "mysql" ? result[0] : result.rows || result;
    const closeDb = async () => await db.$client?.end?.();
    const appliedMigrations = getRows(
      await db[execute](sql.raw(AppliedDatabaseMigrationsQuery)),
    );
    consola.info(
      `Database has \`${appliedMigrations.length}\` applied migration${appliedMigrations.length === 1 ? "" : "s"}`,
    );
    consola.debug(
    ...
    if (
      ...
      appliedMigrations.find(
        (appliedMigration) => appliedMigration.name === args.name,
      )
    ) {
    ...
    if (args.name) {
      await db[execute](
        sql.raw(
          `INSERT INTO "_hub_migrations" (name) values ('${args.name}');`,
        ),
      );
    ...
    consola.info(
      `Found \`${pendingMigrations.length}\` pending migration${pendingMigrations.length === 1 ? "" : "s"}`,
    );
    let migrationsMarkedAsApplied = 0;
    for (const migration of pendingMigrations) {
      const confirmed = await consola.prompt(
        `Mark migration \`${migration.name}\` as applied?`,
        {
          type: "confirm",
          default: true,
          cancel: "null",
    ...
    consola.success(
      `${migrationsMarkedAsApplied} migration${migrationsMarkedAsApplied === 1 ? "" : "s"} marked as applied.`,
    );
    return closeDb();
  },
});
