...
import { createDrizzleClient, splitSqlQueries } from "@syncrolio/nuxthub/db";
...
export default defineCommand({
  ...
  args: {
    query: {
      type: "positional",
      description:
        "The SQL query to execute. If not provided, reads from stdin.",
      required: false,
    },
    cwd: {
      type: "option",
  ...
  async run({ args }) {
    ...
    if (!queryInput) {
      if (process.stdin.isTTY) {
        consola.error(
          "No query provided. Please provide a query as an argument or pipe SQL from stdin.",
        );
        process.exit(1);
      }
    ...
    for (const query of queries) {
      const result = await db[execute](sql.raw(query)).catch((err) => {
        consola.error(
          `Error executing query \`${query}\`: ${err.cause?.message || err.message}`,
        );
        process.exit(1);
      });
      ...
      if (q.startsWith("select")) {
        consola.success(
          `${rows.length} row${rows.length === 1 ? "" : "s"} selected by \`${query}\``,
        );
        consola.log(
          inspect(rows, { depth: null, colors: true, compact: true }),
        );
      } else if (
        q.startsWith("insert") ||
        q.startsWith("update") ||
