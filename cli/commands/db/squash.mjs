...
export default defineCommand({
  meta: {
    name: "squash",
    description:
      "Squash the last X (or selected) migrations into a single migration.",
  },
  args: {
    last: {
      type: "string",
      description:
        "Number of migrations to squash starting from most recently applied. If not specified migrations can be interactively selected.",
      required: false,
    },
    cwd: {
      type: "option",
  ...
  async run({ args }) {
    ...
    if (dropCount !== null && (Number.isNaN(dropCount) || dropCount < 1)) {
      consola.error(
        "Invalid value for --last. Please provide a positive number.",
      );
      process.exit(1);
    }
    ...
    const migrationsDir = join(
      hubConfig.db.migrationsDirs?.[0],
      hubConfig.db.dialect,
    );
    ...
    consola.info(
      `Found ${journal.entries.length} migration${journal.entries.length === 1 ? "" : "s"}`,
    );
    ...
    if (dropCount) {
      ...
      if (dropCount > journal.entries.length) {
        consola.warn(
          `Only ${journal.entries.length} migration${journal.entries.length === 1 ? "" : "s"} available - squashing all`,
        );
      }
      migrationsToDrop = journal.entries.slice(
        -Math.min(dropCount, journal.entries.length),
      );
    } else {
      ...
      const selectedEntries = journal.entries.filter((entry) =>
        selected.includes(entry.tag),
      );
      const oldestSelectedIdx = Math.min(...selectedEntries.map((e) => e.idx));
      migrationsToDrop = journal.entries.filter(
        (entry) => entry.idx >= oldestSelectedIdx,
      );
      ...
      if (migrationsToDrop.length > selected.length) {
        ...
        consola.warn(
          `Including ${additionalCount} additional migration${additionalCount === 1 ? "" : "s"} - all migrations after the oldest selected migration must be squashed.`,
        );
      }
    }
    ...
    consola.log("");
    consola.info(
      `The following ${migrationsToDrop.length} migration${migrationsToDrop.length === 1 ? "" : "s"} will be squashed:`,
    );
    ...
    for (const migration of migrationsToDrop) {
      ...
      const snapshotFilePath = join(
        migrationsDir,
        "meta",
        `${migration.tag.split("_")[0]}_snapshot.json`,
      );
    ...
    const updatedJournalContent = JSON.parse(
      await readFile(journalPath, "utf-8"),
    );
    const newMigration =
      updatedJournalContent.entries[updatedJournalContent.entries.length - 1];

    if (newMigration) {
      const markAsApplied = await consola.prompt(
        `Mark new migration \`${newMigration.tag}\` as already applied?`,
        {
          type: "confirm",
          initial: false,
          cancel: "null",
      ...
      if (markAsApplied === true) {
        ...
        await db[execute](
          sql.raw(
            `INSERT INTO "_hub_migrations" (name) values ('${newMigration.tag}');`,
          ),
        );
        await db.$client?.end?.();
        ...
        consola.info(
          `Run \`npx nuxt dev\` or \`npx nuxt db migrate\` to apply the new migration. Alternatively, to mark the migration as already applied, run \`npx nuxt db mark-as-migrated ${newMigration.tag}\`.`,
        );
      }
    }
  },
