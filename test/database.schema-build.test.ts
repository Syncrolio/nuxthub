...
describe("buildDatabaseSchema", () => {
  it("bundles cache-based schema paths so output stays importable", async () => {
    ...
    try {
      ...
      await writeFile(
        entryPath,
        `export * from '${toImportSpecifier(entryPath, schemaSource)}'\n`,
      );
  ...
  it("resolves @syncrolio/db/schema imports to local schema entry during build", async () => {
    const rootDir = await mkdtemp(
      join(tmpdir(), "nuxthub-schema-build-alias-"),
    );
    ...
    const externalSchemaPath = join(
      rootDir,
      "node_modules/@syncrolio/db/schema.mjs",
    );

    try {
      await mkdir(join(buildDir, "hub/db"), { recursive: true });
      await mkdir(join(buildDir, "local"), { recursive: true });
      await mkdir(join(rootDir, "node_modules/@syncrolio/db"), {
        recursive: true,
      });

      await writeFile(
        localSchemaPath,
        'export const pages = { id: "local-pages" }\n',
      );
      await writeFile(
        localConsumerPath,
        "import { pages } from '@syncrolio/db/schema'\nexport const pageIdFromAlias = pages.id\n",
      );
